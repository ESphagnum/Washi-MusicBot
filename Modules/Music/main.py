...
import base64
import io
from Config.Music.config import *
...
async def _join(ctx, from_cmd=False, author=None):
    ...
    if os.path.isdir(save_path + f"/tempMusic/{ctx.guild.id}"):
        shutil.rmtree(save_path + f"/tempMusic/{ctx.guild.id}")
...
class PlayerControlView(discord.ui.View):
    ...
    async def loopqueue(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user not in self.voice_state.voice.channel.members:
            return await interaction.response.send_message(loc["messages"]["not_in_same_voice"], ephemeral=True)
        await interaction.response.defer()
...
class Music(commands.Cog):
    ...
    def cog_unload(self):
        ...
        try:
            shutil.rmtree(save_path + f"/tempMusic")
        except:
            pass
    ...
    async def playfile(self, ctx, *, title=None):
        ...
        if not os.path.isdir(save_path + f"/tempMusic"):
            os.mkdir(save_path + f"/tempMusic")
        if not os.path.isdir(save_path + f"/tempMusic/" + str(ctx.guild.id)):
            os.mkdir(save_path + f"/tempMusic/" + str(ctx.guild.id))
        # Path is save_path + f"/tempMusic/<guild_id>/<current_time>.<extension>
        filename = save_path + f"/tempMusic/"+ str(ctx.guild.id) + "/" + str(int(time.time() * 10000000)) + "." + attachment.filename.split(".")[-1]
